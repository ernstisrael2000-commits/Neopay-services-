...
export default function CryptoMarketView({ client, onRequestAuth }: CryptoMarketViewProps) {
  ...
  const submit = async () => {
    ...
    if (!hasFunds) { toast.error('Votre solde Solutionpam est insuffisant pour cette commande.'); return; }
    setSubmitting(true);
    try {
  ...
  return <div className="space-y-5">
    <section className="overflow-hidden rounded-3xl bg-[#102a43] p-5 text-white shadow-xl shadow-[#102a43]/20">
      <div className="flex items-start justify-between gap-3">
        <div><div className="mb-3 flex h-11 w-11 items-center justify-center rounded-2xl bg-white/10 ring-1 ring-white/20"><Coins className="h-5 w-5 text-cyan-200" /></div>
          <h2 className="text-xl font-black tracking-tight">Marché Crypto Solutionpam</h2>
           <p className="mt-1 max-w-md text-xs font-medium leading-relaxed text-[#d9f0ee]/80">Achetez avec votre solde Solutionpam. Un opérateur vérifie chaque commande avant l’envoi.</p>
        </div>
    ...
    {loading ? <Loading /> : tab === 'history' ? (
      ...
      <div className="space-y-5">
         {client && <div className="flex items-center justify-between rounded-2xl border border-[#b9eee8] bg-[#effaf8] p-3.5"><div className="flex items-center gap-3"><Wallet className="h-5 w-5 text-[#167d73]" /><div><p className="text-[10px] font-black uppercase tracking-widest text-[#167d73]">Solde Solutionpam disponible</p><p data-testid="text-crypto-balance" className="mt-0.5 text-lg font-black text-[#102a43]">{formatUSD(balance)}</p></div></div><ShieldCheck className="h-5 w-5 text-[#167d73]" /></div>}
          {!client && <div className="flex items-start gap-3 rounded-2xl border border-[#b9eee8] bg-[#effaf8] p-3.5"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-[#167d73]" /><div><p className="text-sm font-black text-[#102a43]">Connexion requise</p><p className="mt-0.5 text-[11px] leading-relaxed text-[#167d73]">Votre solde Solutionpam protège votre commande et son suivi.</p><button type="button" onClick={onRequestAuth} className="mt-2 text-xs font-black text-[#167d73] underline underline-offset-2">Se connecter</button></div></div>}
     ...
     {confirming && selectedCrypto && selectedNetwork && <div className="fixed inset-0 z-[100] flex items-end justify-center bg-[#102a43]/60 p-0 backdrop-blur-sm sm:items-center sm:p-5"><div className="w-full max-w-md rounded-t-3xl bg-[#fffdf8] p-5 shadow-2xl sm:rounded-3xl"><div className="mb-4 flex items-start gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-[#effaf8] text-[#167d73]"><ShieldCheck className="h-5 w-5" /></div><div><h3 className="font-black text-[#102a43]">Confirmer l’achat avec Solutionpam</h3><p className="mt-1 text-xs leading-relaxed text-slate-500">Cette somme sera débitée immédiatement de votre solde Solutionpam. L’envoi crypto sera traité manuellement.</p></div></div><div className="space-y-2 rounded-2xl bg-[#f4f1e9] p-4 text-sm"><Row label="Vous recevez" value={`${amountValue} ${selectedCrypto.symbol}`} accent /><Row label="Réseau" value={selectedNetwork.networkName} /><Row label="Total débité" value={formatUSD(totalUSD)} strong /></div><div className="mt-4 grid grid-cols-2 gap-2"><Button type="button" variant="outline" onClick={() => setConfirming(false)} className="h-11 rounded-xl font-bold">Retour</Button><Button type="button" onClick={submit} disabled={submitting} className="h-11 rounded-xl bg-[#167d73] font-black text-white">{submitting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <CheckCircle2 className="mr-2 h-4 w-4" />}Confirmer</Button></div></div></div>}
  </div>;
}
