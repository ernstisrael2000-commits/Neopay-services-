...
Didit verification for this gate proves the official owner of the SolutionPAM account. Names printed on HeyQO cards are never part of authorization, even when one account manages cards with different holder names.
...
**How to apply:** Keep card security state in the server-only card security collection, match Didit to the account identity fingerprint, gate every `/api/client/cards` data or mutation route, require the PIN again for sensitive detail reveal, avoid persistence/logging of full details, and clear the grant on client logout.